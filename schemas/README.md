...
- [`tests/test_check_x_not_y.py`](../tests/test_check_x_not_y.py) runs every fixture through the tool's own loaders too, so a valid fixture must load and an invalid one must stop the run. It also checks that each schema names the same judgments, registers, key form and white space as the tool, so the schemas and the tool cannot drift apart. The tool reads each value as its schema does: a line is JSON Schema's `integer`, so `12.0` is line 12; a key matches its form to its very end; and a reason or a basis must hold a character that is white space in neither ECMA-262's `\s`, which the schemas' patterns use, nor Python's.
