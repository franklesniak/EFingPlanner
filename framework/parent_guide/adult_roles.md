...
- **The must-do list and within-day order.** Within your guardrails (approved cities, budget band, pacing and safety, booking availability), your child picks which sights make the must-do list and the order of their day. Their picks carry real weight. In a multi-generational party, group agreement may sometimes reshape one -- but only with a reason given to the child, **never silently.**
- **The one personal-interest pick.** The single thing your child most wants is honored unconditionally. You may block it only for three reasons: it costs more than the budget band, it cannot be booked, or it is not safe or doable for everyone (including a lower-stamina traveler). A group vote does **not** override it. Choose it together, wisely, so the promise is keepable -- steer gently toward an affordable, bookable pick, and show the three blocks before your child commits.

Record both on the child's "My Calls" page, with a one-line adult acknowledgment beside each, so the authority is visible.
