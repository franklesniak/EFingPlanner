...
- **Keep the session choreography exactly as written.** The seven mandatory fields, the `Stuck? Use the...` sentence and the `Finished? Use the...` sentence are a designed executive-function support. A child meets the same words each time, so they are never varied, and the contraction and dash rules above do not apply to them. Both are identical in all 14 child sessions. (`00_parent_setup.md` is parent-facing and carries none of this choreography.)
