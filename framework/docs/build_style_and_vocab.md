...
- **Seven places keep the full form:** headings, field names, table cells, worksheet labels, and nav lines; a standing rule the child must obey; safety rules and privacy prohibitions; agreed labels and canonical concept Names; text the child copies, such as an example search phrase; quoted boilerplate and quoted banned phrases; and parent-facing or builder-facing files, whose register this convention does not change.
...
- **Dash budget.** In a child-facing file: at most one prose dash per `##` section, none at all in Goal, Start Here, Stop Point, or a worksheet table, and per file at most 8, or one per 5 prose lines, whichever number is smaller. A parent-facing file gets the same per-file number with no per-section cap. A builder or spec file gets one per 4 prose lines and no per-section cap, because a `##` section here is a chapter of ten bullets while a session section is four lines, so a per-section cap would not mean the same thing in the two places.
- **How to count any density device.** These rules apply to the dash budget, the `real`/`genuine(ly)` cap, and the `X, not Y` cap alike. **Count once:** a paired parenthetical `-- like this --` is one device, even though it is spelled with two dash characters. Counting it twice would penalise the clearer paired form and reward a trailing dash. **Do not count:** anything inside a code span, a fenced block, a block quote, or a table cell; a label separator (`- **Term** -- definition`); or a bold label lead-in, which names a rule rather than being prose. A rule that quotes the thing it governs has to be able to quote it. **Content outranks the budget.** A rule's own subject matter, a Named concept, and any calibration pair the archived spec requires are content, never density. Never delete a required rule, a safety statement, or a calibration pair to make a count smaller; rework neighbouring prose instead, or leave the count high and say why. Say why in the form `<!-- density-exempt: <device> -- <reason> -->` on the line above, so that a future gate can honour the exemption instead of flagging it and so that a reader can see the judgement rather than guess at it. A gate cannot tell a required calibration pair from an ordinary contrast, which is what makes the marker necessary rather than merely tidy. This file is builder-facing and is measured under the builder caps.
...
- **`guardrails` stays, and stays adult-facing.** Its first use in any file names its contents in the same sentence or the one above ("budget, approved cities, pacing, safety, availability"). It never appears in a child-facing file.
- **Keep the session choreography exactly as written.** The seven mandatory fields, the `Stuck? Use the...` sentence, the `Finished? Use the...` sentence, and the opening words `If you have extra energy` are a designed executive-function support. A child meets the same words each time, so they are never varied, and the contraction and dash rules above do not apply to them. **What is frozen is the shared sentence, not the whole line, and the list above is the whole of it.** Three things are deliberately not frozen, and a later author or density gate must not normalise them. (a) A session may add its own sentence after the card pointer; Session 04 does, and that trailing sentence is ordinary child-facing prose which the rules above do govern. (b) The close of the extra-energy line is `If not, you are done.` by default, but a session that can name a definite finished state says so instead, as Sessions 15 and 21 do. (c) A session whose extension continues past First Taste has no close at all, as in Sessions 05, 09 and 33, because there is nothing to be done with. Measured across the built sessions: the `Stuck?` sentence is identical in 14 of 14, the `Finished?` sentence in 14 of 14, and the default close appears in only 8 of the 13 sessions that have the section.
