...
- **The rule-card test settles a borderline case.** Print the sentence alone on a card. If it still makes sense as a rule, it is a rule: keep `do not`, and prefer `Never` for the strongest prohibitions. Otherwise read it aloud the way a kind adult speaks to a ten-year-old, and contract it if the full form sounds like a form letter.
- **Spaced dashes: four tests, in order.** Is it a label separator (`- **Term** -- definition`)? Keep it, and do not count it. Does it introduce a list or restate what was just named? Use a colon. Does the text after it have its own subject and verb? Make it a new sentence, and contract that sentence in the same edit. Is it a short verbless aside and the only one in its section? Keep it. Otherwise use a comma, or cut the aside. **The tests apply to every spaced dash a reader sees, wherever it sits.** Only test 1's label separator is exempt from the tests themselves; every other exemption in this section is a *counting* exemption, and being uncounted does not license a dash that fails a test. A dash inside a bold lead-in is the case that catches people out: it is invisible to the budget and still governed by the four tests.
