...
<!-- density-exempt: X, not Y -- each bullet states a voice rule as the contrast it enforces, and a law that states a rule is its own subject matter -->
...
<!-- density-exempt: X, not Y -- these bullets define the density devices, their counting rules and their exemptions, and stating those rules needs the contrasts they rule on -->
...
- **How to count any density device.** These rules apply to the dash budget, the `real`/`genuine(ly)` cap, and the `X, not Y` cap alike. **A prose line is what every ratio here divides by.** It is a non-blank line of authored text: a paragraph, a list item, or a block quote that counts under the rule below. Outside the prose-line count are headings, table rows including the delimiter row, fenced blocks together with their fence lines, and a line holding nothing but an HTML comment. A line that merely ends in an inline HTML comment is still a prose line, because the prose in front of the comment is what a reader sees. **Count once:** a paired parenthetical `-- like this --` is one device, even though it is spelled with two dash characters. Counting it twice would penalize the clearer paired form and reward a trailing dash. **Do not count:** anything inside a code span, a fenced block, or a table cell; a label separator (`- **Term** -- definition`); or a bold label lead-in that holds no spaced dash and no `X, not Y` contrast of its own, which names a rule rather than being prose. A bold run carrying its own spaced dash is two clauses rather than a label: that dash is counted and tested like any other, and a counter that strips the whole bold run will not see it. **A block quote is a prose line unless it is a quotation.** Exempt it only when its content is verbatim borrowed text and the page shows that on its face with enclosing quotation marks, as the parent guide's ten coaching scripts do. Name the source, if you name it, on its own line after the block quote: the recount reads a quotation by its marks alone. A block quote used as a callout or a carry-over tag renders to the reader as ordinary prose, so it counts, and its lines join the prose-line count the ratios divide. Session 04 settles the case: its Carry-over tag holds the only spaced dash in the file, and a container-based exemption made that dash invisible. A rule that quotes the thing it governs has to be able to quote it. **Content outranks the budget.** A rule's own subject matter, a Named concept, any calibration pair the archived spec requires, and any wording a build brief fixes for a page are content, never density. Never delete a required rule, a safety statement, or a calibration pair to make a count smaller; rework neighboring prose instead, or leave the count high and say why. Before a density rewrite, read what the page's build brief and the spec require of it. Say why in the form `<!-- density-exempt: <device> -- <reason> -->` on the line above, so that a future gate can honor the exemption instead of flagging it and so that a reader can see the judgment rather than guess at it. A gate cannot tell a required calibration pair from an ordinary contrast, which is what makes the marker necessary rather than merely tidy. Name the device as this file names it: `spaced dash`, `real`, or `X, not Y`. A marker covers the block directly below it: one paragraph, one whole list, or, above a heading, that heading's section. Text above a page's first `##` heading is not a `##` section, so only the file caps reach it. **Register can change at a heading.** A child-facing session carries two parent-facing regions, the labeled "For parents" strip and `## Parent Notes`; both are measured under the parent-facing caps, and the ban on `genuine` stops at their headings. The rest of the file is child-facing. Per-file caps still follow the file's own register. This file is builder-facing and is measured under the builder caps.
...
- **`X, not Y` is capped, not banned.** It pre-empts a misreading, which is an instructional move. Keep the instance where a reader actually makes the mistake and the mistake stops them. Cap: 2 per child-facing file, 3 per parent-facing file, 4 per builder or spec file, and one per `##` section. Rewrite each instance over the cap with a *different* technique each time -- state it positively; split the negation into its own short sentence (once per file at most); name the actor or the consequence; or turn it into a question the child answers. A page that needs more rewrites than there are techniques may repeat any of them except the split negation. The device is any sentence that rejects a named alternative: `X, not Y`, `X rather than Y`, `X instead of Y`, `X, and never Y`, and a fragment such as `Not a failure.` that follows a claim. The one-per-section cap applies in child-facing and parent-facing text alike, a split negation counts toward its once-per-file limit wherever it appears, and `It doesn't X. It Ys.` is the banned `It's not X. It's Y.` shape. A release counts as a split negation only when the next sentence recasts what the thing is. **Never** write "It's not X. It's Y.", as two sentences or joined into one. Do not repeat the same contrast shape in two consecutive First Taste sessions.
- **`guardrails` stays, and stays adult-facing.** In any file under `framework/` or `destinations/`, its first use in prose names its contents in the same sentence or the one above ("budget, approved cities, pacing, safety, availability"); `framework/parent_guide/coaching_and_support.md` is the model. A table cell is a label, so a cell may carry the term before the gloss arrives, as the roles table does. The archived spec is outside this rule. It never appears in child-facing prose.
...
- **The Optional Extension is choreographed but not frozen, and the difference is the point.** Every child session has the section. Four things vary on purpose, and a later author or density gate must not normalize them. (a) A session may add its own sentence after the `Finished?` card pointer; Session 04 does, and that trailing sentence is ordinary child-facing prose which the rules above do govern. (b) The opener is `If you have extra energy` in every child session except the capstone, Session 53, which opens `If you want to keep going`, because a finish line is not a mid-course session and should not sound like one. (c) The close is `If not, you are done.` unless the session can name a definite finished state and says that instead, as Sessions 15, 21 and 31 do. (d) Four sessions have no close at all -- 05, 09, 33 and 53 -- because their extension continues past First Taste, so there is nothing to be done with. **Measure by section, not by phrase.** Every exception above was found by listing each `## Optional Extension` heading and reading the line under it. The three earlier versions of this rule were each wrong, because each searched for the wording it expected to find and so could not see the sessions that word it differently.
...
- Every fenced code block declares a language. Reserve a `text`-tagged fence for genuinely preformatted content (directory trees, a single worked formula). (MD040)
...
The archived spec teaches a load-bearing idea: an honest "we have not decided that yet" is a **valid, complete answer** on a planning worksheet, so the child can leave it that way. The spec writes that idea using the literal placeholder token that software projects use. The built curriculum may not, because the prohibited-placeholder hook bans that token under `framework/` and `destinations/` -- and because the software token is out of register for a ten-year-old anyway.
