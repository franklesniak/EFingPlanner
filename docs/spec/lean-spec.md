...
The original combined archive remains at [Unified Specification](specification.md), from which this file is derived. Those three archived spec files -- the combined archive, this Lean split, and the [Full/OER Companion](full-oer-companion.md) -- overlap by design and are treated as a frozen set: an edit to one should be reconciled across the others to avoid drift. Use the companion only when rebuilding the curriculum for another destination or publishing it as an open educational resource.
