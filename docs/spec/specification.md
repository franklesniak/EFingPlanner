...
**Spec version:** 9.1 -- a post-loop **defect-fix erratum** to v9.0, the **terminal in-loop revision**. The expert-review loop concluded at v9.0; v9.1 does not reopen it -- it applies the worksheet-form defect fixes the built repo surfaced (see the revision history), and further improvement still flows through the built repo (the end-of-life statement in the Split plan, "How to read"). The full version block, the revision history, and the spec-maintenance note (for whoever *edits this spec*) live in the appendix at the very end of this document.
...
- **Worksheet forms are compact Markdown tables, not monospace underscore blocks.** Use a two-column label-and-answer table for a single-record form (the label in the first cell, the child's answer in the second -- the built templates use the header pair `Prompt | Your answer`); use a small comparison grid (criteria as rows, options as columns) for scoring or compare worksheets. The empty cells are the fill-in space. A real Markdown table reflows on any screen, prints as bordered cells you can handwrite in, and becomes an editable table when the page is copied into Google Docs; an ASCII/underscore fill-in block inside a code fence does none of these -- it does not reflow, and it collapses into blank-line-separated paragraphs when pasted into Google Docs. Reserve fenced `text` blocks for content that is genuinely preformatted (directory trees, a single worked formula), not fill-in forms.
...
| Hotel -- each other city you kept (one row per city) | $____ per night x ____ nights x ____ rooms | |
...
**Spec version:** 9.1 (post-loop defect-fix erratum to v9.0). v9.0 (ninth expert-criticism revision) was the **terminal in-loop revision**: a finishing pass that repairs the round-9 defects and stale cross-references, completes the First Taste supporting machinery, physically reorders the document Lean-first, re-anchors the split to build start with end-of-life/canonicity defined, and closes a handful of verify-framed trip-content gaps for this party shape. **v9.1** does not reopen the expert-review loop; it is the kind of post-terminal defect fix the shrink-only rule anticipates (see the v9.1 revision-history row). This spec mandates a `framework/CHANGELOG.md` for the built curriculum (Section 30.6); it eats its own dogfood with a version and a compact revision summary of its own (full per-round diffs are in git history).
...
- **v9.1 (post-loop defect-fix erratum to v9.0).** A defect fix, not a new review round -- the first revision under the shrink-only clause below. It applies the worksheet-form corrections the built repo surfaced: **Section 12.1** now mandates reflow-friendly Markdown tables for worksheet forms (a two-column label-and-answer table -- the templates use `Prompt | Your answer` -- or a narrow criteria-by-option grid for scoring), and the **Section 23** budget worked example models each city's hotel as its own additive row so a two-city plan is neither double-counted nor dropped. **Removed:** the monospace underscore fill-in blocks these replace (they do not reflow and collapse to blank-line paragraphs when pasted into Google Docs). Adds no new artifact, tracker, or mechanic -- a load-bearing correctness/usability fix, net-small, and the whole-document shrink-only rule still holds.
