...
- **Do not edit** `docs/spec/*`, `CLAUDE.md`, `.github/copilot-instructions.md`, `.github/instructions/*`, or any other governance/instruction file. This build works only within the curriculum tree: it *creates* curriculum files and — where this batch's scope requires it (for example, Batch 1's concrete→insert upgrade of the shared sessions per §29.1, or the final whole-repo consistency pass per §31) — *edits already-built curriculum files*; it never edits `docs/spec/*` or any governance/instruction file.
...
- Self-check before stopping: grep the built sessions for the leak tokens — `grep -rwE 'Chicago|ORD|17|grandmother|uncle' framework/sessions/` (the explicit `framework/sessions/` target + `-r` scans the built files instead of reading stdin; `-w` gives portable standalone-token matching without the non-POSIX `\b` escape) — and confirm no hard-coded family value (blanks pointing to `trip_basics.md` are fine); confirm every session has a named artifact and a stop point (§15).
