...
- **Scope:** Agent-specific entry point for Hermes Agent and compatible AI coding agents operating in this repository. Mirrors a minimal inline summary of the highest-priority shared rules; `.github/copilot-instructions.md` remains the canonical source of truth.
...
This file provides project-specific instructions for Hermes Agent and compatible AI coding agents operating in this repository. These instructions ensure that agents follow the same coding standards, safety rules, and workflows that apply to all contributors.
...
This file intentionally keeps only a minimal inline summary of the highest-priority shared rules so that Hermes receives critical guidance immediately. The full shared rule set remains in the canonical file above.
...
This section is retained as Hermes host-specific protocol. Thin-entry-point pruning must preserve it unless the repository owner explicitly waives Azure DevOps PR review protocol for the retained Hermes entry point.
...
- Hermes has no autonomous Azure DevOps wake-up. Mentions route Azure DevOps comments only when the user's runtime explicitly forwards them into the active Hermes session.
- When Azure DevOps connector/API tooling is available and safely authenticated, Hermes MAY inspect PR reviewers, threads, comments, thread status, and PR statuses through Azure DevOps REST APIs, and MAY post replies/comments, update thread status, or create PR statuses. When tooling is missing or insufficient, state the needed manual owner action instead.
