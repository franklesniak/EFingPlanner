...
- **Gender/age neutrality – a standing invariant, not a one-off task.** Every batch of built curriculum must stay gender-neutral and age-flexible (~9–11): a generic child is "your child / the child / they", child-facing text stays second-person "you", the audience statement is always the range, and no page sets a hard age bar. This does not stop being true once it is written down somewhere; it is checked by the QA grep on every batch. The design spec's prose is already neutralized.
- **Neutral-pronoun requirement: recorded.** `framework/docs/build_style_and_vocab.md` now carries the neutral-pronoun rule, the age-flexibility rule (the audience statement is always the range "roughly 9-11"; a loose reference to a typical reader is fine in parent- and builder-facing text; no page sets a hard age bar), and a portable QA grep. The remaining half of this bullet (README audience wording) is checked in the closing consistency pass.
- **The curriculum build itself:** follow the spec's §31 order — the file-cut (offloaded in section B), then the **Batch 0 design-validation pilot**, then the build itself. The selected track is the **Full / OER Build**, which is what the build briefs in `docs/build/` target; earlier wording here said "Lean Build" and is superseded. The track matters: the Full Build adds the destination-neutral skeleton plus insert upgrade, and it keeps the second "verify the built slice" gate, which the Lean Build folds into Batch 0. Batch 0 and the child-facing parts of the build are **maintainer + child**-led (they require piloting with the actual child), not pure coding-agent tasks. Scope them as their own issues as you go.
