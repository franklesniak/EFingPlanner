...
def strip_html_comments(
    ...
    ``.github/scripts/check-prohibited-placeholders.py``. A comment ends where
    the page ends it, which is ``split_html_comments``'s walk.
    """
...
def following_comment_end(contents: Sequence[str], row: int, index: int) -> tuple[int, int]:
    ...
    caller is right to read the characters after it as ordinary text. Nor is
    one whose text the production refuses, which is what ``<!--`` with any
    later ``-->`` used to be read as.

    The rows are joined with the line endings they had and handed to
    ``inline_comment_end``, so this hook and its sibling read one comment
    production and not two. Kept in step with the helper in
    ``.github/scripts/check-readability.py``.
    <https://spec.commonmark.org/0.31.2/#raw-html>
    """
    ...
    while row < len(contents):
        ...
        if is_in_comment:
            ...
            if closer is None:
                ...
                index = 0
                continue
            spans[row].append(line[index : closer.end()])
            index = closer.end()
            is_in_comment = False
            continue
        ...
        if character == "<":
            if line.startswith("<!--", index):
                ...
                if close_row == -1:
                    index += len("<!--")
                    continue
                # The page ends it (``comment_extent``): an empty comment
                # where it stands, and any other at ``-->`` or ``--!>``.
                _text_end, comment_end = comment_extent(line, index)
                if comment_end != -1:
                    spans[row].append(line[index:comment_end])
                    index = comment_end
                    continue
                spans[row].append("<!--")
                index += len("<!--")
