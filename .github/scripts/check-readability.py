...
#: ``[x](fo<0x01>o "<!-- audience: adult -->")`` the page makes the marker a
#: ``title`` attribute, and reading it as a comment took a child-facing
#: document out of the reading gate in silence.
#:
...
def inline_link_end(line: str, open_index: int) -> int:
    ...
    if index < length and line[index] == "<":
        ...
        while index < length:
            ...
            if character == "(":
                depth += 1
                # The bound ``_LINK_DESTINATION`` carries, in the other
                # spelling of the same rule. Both say what a destination may
                # hold and this one said nothing about nesting at all, so at
                # 33 levels the pattern refused an image and this scan
                # accepted one: ``![<!-- audience: adult -->](a(a(...)))`` was
                # masked here as an image's description while markdown-it
                # 14.3.0, micromark 4.0.2 and the production renderer left the
                # image literal and read the marker as the comment it is, and
                # an adult-facing document was scored as a child's page.
                # Measured from 30 levels to 40 on all three: the target forms
                # at 32 and does not at 33.
...
def paragraph_metadata_skips(
    ...
    contents = [line.text[line.content_start :] for line in lines]
    definition_rows = {
        row
    ...
    scanned = [
        ParagraphLine(
            start=line.start,
            text=line.text[: line.content_start].ljust(len(line.text)),
    ...
    _, code_spans = scan_paragraph_inlines(scanned, None)
    masked = [list(line.text) for line in scanned]
...
def scan_document_inlines(text: str) -> DocumentInlines:
    ...
    for number, raw_line in enumerate(lines):
        ...
        fence_line = normalize_for_fence_opening(
            line, list_contexts, paragraph_open, previous_path
        )
...
def extract_prose(text: str) -> str:
    ...
    # This walk's paragraph model, and it is as small as the one rule that
    # needs it. ``collect_reference_labels`` keeps the same two fields for the
    # same helper.
    paragraph_open = False
    previous_content = ""
    ...
    for index, raw_line in enumerate(lines):
        ...
        # The paragraph state the line above left, read before it is advanced,
        # because the table rule below asks what stood above the *header* and
        # the header is this line. Advanced here rather than at the foot of the
        # loop so that every exit below leaves it right; every exit above is a
        # line inside a fence or a raw-text run, and each of those clears it
        # where it stands.
        header_paragraph_open = paragraph_open
        paragraph_open = opens_a_paragraph(
            fence_line.content, paragraph_open, previous_content
        )
        previous_content = fence_line.content
