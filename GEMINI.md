...
This section is retained as Gemini host-specific protocol. Thin-entry-point pruning must preserve it unless the repository owner explicitly waives Azure DevOps PR review protocol for the retained Gemini entry point.
...
- Gemini has no autonomous Azure DevOps wake-up. Mentions route Azure DevOps comments only when the user's runtime explicitly forwards them into the active Gemini session.
- When Azure DevOps connector/API tooling is available and safely authenticated, Gemini MAY inspect PR reviewers, threads, comments, thread status, and PR statuses through Azure DevOps REST APIs, and MAY post replies/comments, update thread status, or create PR statuses. When tooling is missing or insufficient, state the needed manual owner action instead.
