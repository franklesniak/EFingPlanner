...
Claude MUST follow [Shared Review Governance](.github/copilot-instructions.md#shared-review-governance) for complete finding inventory, per-finding weighted decisions, paired remote reviews, attribution, bounded recovery, CI repair, deferrals, and continuation. When feedback arrives from Copilot, remote Codex, a human, or any other reviewer, apply that contract and this platform protocol to each distinct finding.
...
1. **Signal processing.** Inspect the runtime's actual capabilities. When review-comment reactions are supported, add the literal API reaction `eyes` when work begins and remove it after completion or evidence-backed refutation. Do not assume a fixed MCP tool inventory; use supported equivalent tools. If reactions are unavailable, skip them silently.
...
4. **Build an evaluation rubric.** Define and display a fresh weighted rubric with 4–6 scoring criteria relevant to the concern (for example, style-guide compliance, correctness, security, performance, maintainability, code simplicity, PII safety, PS 5.1 compatibility, test reliability, user impact, backward compatibility, or long-term clarity). Finish the options before defining the rubric. Display criteria, weights, and their rationale before scoring each criterion on a 1–5 scale. Apply Rubric weighting below. Take the time needed to ensure the rubric is **comprehensive and defensible**: each criterion should be one a reasonable maintainer would accept as relevant, and the criteria collectively should cover the substantive technical considerations of the concern, not just surface-level ones.

5. **Score and select.** Apply the rubric to every option, taking the time needed to score each criterion defensibly. Present the results in a Markdown table. Display each criterion's weight and every option's checked weighted total before selection. Select the highest-supported eligible option; use the shared technical-tie rule when necessary. When the rubric produces a clear highest-scoring option, the agent **MUST** select that option and carry it forward to step 7, unless an escalation condition under the **Operationalized escalation gate** in Escalation protocol below applies. The conditions defined in the **Operationalized escalation gate** continue to operate against weighted totals when weighting is applied. A topic touching owner preferences, governance, or policy is not, by itself, an escalation trigger when the rubric produces a clear winner; this clarification stands independently of the protected-file authorization checkpoint in step 7.

6. **Post the evaluation.** Before editing, publish the complete shared finding evaluation with options, fresh rubric, score table, selected action, references, tests, and intended placement. After implementation, reply with the resulting PR-head SHA and validation evidence.
...
8. **Evaluate style guide impact.** Read the full applicable guide and follow the shared prevention and deferral rules. If no guide change is warranted, record that determination and its reason; no prompt is required. When a guide change is warranted, implement it when explicit current-task authority covers it. Otherwise, post the required ready-to-file fenced prompt. For a GitHub inline finding, use the same native review thread. For a GitHub body-only finding, use a standalone PR comment that records its synthetic finding key, source review identity, reviewed commit, and location when available. Record the prompt comment's native identity, keep the guide action pending until attributable disposition, and continue independent authorized work.
...
The PR-scope explicit-boundary escalation under condition (d) of the **Operationalized escalation gate** in Escalation protocol below is unchanged: an explicit scope sentence in the PR description that the selected fix would directly violate still triggers escalation regardless of weighting.
...
(a) After bounded evidence gathering and the shared technical-tie rule, the remaining choice requires an owner preference, new authority, or a material intended-outcome change. A tie alone does not trigger escalation; choose the simpler reversible option when the remaining options are equally safe and correct.

(b) A decisive fact remains unresolved after bounded primary research, focused testing, or independent review, and selecting without it would cross a safety or authority boundary. A merely missing technical score first requires investigation, not an automatic owner question.
...
When this active session creates a pull request, or a supported runtime route delivers a PR-created event or the owner's `@claude start review loop` comment, initiate the following cycle. Do not claim automatic wake-up when event routing is unavailable.

### Loop procedure

1. **Request the reviewer pair.** Capture complete native request, review, inline-comment, conversation, and relevant-run baselines, the head and material PR description for each service. Prefer supported Copilot Balanced selection and accept Lite fallback. Use `request_copilot_review` or a supported equivalent; CLI fallback is `gh pr edit PR-NUMBER --add-reviewer '@copilot'`. Confirm delivery through fresh authenticated evidence before sending the exact remote Codex trigger `@codex review`. Follow the shared serialization and bounded recovery table; an accepted request is not completion.
2. **Wait for both reviews (active polling).** Apply the shared independent per-service counters, five-observation body limit, attribution, and terminal-failure recovery rules. Immediately after requesting the reviews, begin an active poll loop — do **not** rely solely on webhook delivery, which may be delayed or never arrive. The poll loop **MUST** follow Polling protocol below.

3. **Check review bodies and coverage.** Retrieve both submitted review bodies, suppressed/advisory findings, all-state inline threads, and attributable conversation results. Reconcile declared counts. Apply the shared five-observation missing-body bound; an unavailable summary is not zero findings. Report partial file coverage when known, for example, `Copilot reviewed 7 of 9 changed files; the remaining files need additional inspection.` Partial coverage alone does not require another request.
4. **Check completion.** One zero-comment review does not complete the loop. Require both attributable clean reviews on the final unchanged input, complete earlier dispositions, successful required CI, and any required independent checks. An acknowledgment, failed service, unknown result, timeout, or exhausted retry remains incomplete. Review completion grants no new merge authority.
5. **Process each finding.** Follow Handling Code Review Comments and the shared decision process for every real finding. A protected-content authorization question pauses only the dependent action; continue independent findings and CI repair. Resolve completed threads or state the manual action when tooling is absent. Clean up temporary processing reactions.
6. **Handle prevention and deferrals.** Implement guide changes already covered by explicit authority. For uncovered changes, post the ready-to-file prompt and narrow question and keep that action pending. Apply the shared deferral rules and whole-PR sweep; a guide recommendation does not stop independent authorized work.
7. **Re-request review.** Before re-requesting, the agent **MUST** verify that the final fix commit(s) for the current round that are intended to land on the PR head are reachable from the PR's head ref. The agent **MUST** record those PR-head fix commit SHA(s) after any merge, rebase, or cherry-pick that changes commit IDs; intermediate authored commit SHA(s) that were superseded by equivalent PR-head commit SHA(s) **MUST NOT** block re-requesting review on their own. Apply Direct PR-head placement during an active loop below.
...
- **Detection criterion.** On each poll, use **both** `get_reviews` **and** `get_review_comments` as co-equal detection signals. A new review round is detected when **either** of the following is true:
  ...
  If no baseline exists, a new bot record is a discovery signal only. Check authenticated actor, native identity, request linkage, timing, and head before attributing it. Fresh inline findings permit immediate work in steps 3 and 5 without waiting for a summary, but the full body and both completed reviews remain required. Observe remote Codex through its authenticated conversation, review, and run evidence independently.
- **Co-equal discovery and complete observations.** Either successful source can reveal a finding for immediate work even when another source fails. It cannot establish a complete inventory or clean review by itself. A successful empty observation requires every relevant source to be parsed and pagination-complete. If one source is empty and another fails, the cycle is indeterminate and uses the bounded observation recovery under Safety limits.
- **Independent timeouts.** Apply the shared ten-successful-observation limit independently to Copilot and remote Codex. Each complete observation without attributable current-input completion advances that service's count, including repeated stale events and observations after the other service is clean. Failed observations neither advance nor reset the count. At ten, pause that incomplete gate and name the service and evidence. Continue independent work; do not report timeout as success.
- **State tracking (recommended).** On each confirmed-successful no-review poll, update a visible progress indicator in the session transcript (for example, a todo-list entry such as `"Round N: awaiting SERVICE review, complete pending observation M/10"`) so that stalls are observable. Failed cycles use the visible failure lines described under **Safety limits** instead of advancing `M`.
...
When all intended fixes are reachable, reconcile older in-flight requests and apply the shared input-invalidation rules before returning to step 1. A new head requires both reviews. A material description change invalidates affected reviews. An unchanged input permits a request only for an evidenced bounded retry; a new round, refutation, status update, or resume alone does not justify a duplicate.
...
When the PR owner posts `@claude resume review loop` through a supported runtime route, recover scoped authority, input identities, both service states, finding dispositions, fix reachability, and retry counters. Finish authorized paused fixes and reconcile pending operations before any new request. The invocation round counter and timeout can reset; same-input delivery and service-attempt budgets cannot reset to evade exhaustion. A mention or subscription is a wake-up mechanism only when the actual Claude runtime supports it; keep the required active poll loop while running.
